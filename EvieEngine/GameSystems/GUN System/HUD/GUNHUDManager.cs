...
namespace EvieEngine.GUNSystem.HUD
{
    public class GUNHUDManager : MonoBehaviour
    {
        [Title("Привязки")] public GunInput gunInput;
        ...
        [Title("Настройки")] public float fadeSpeed = 5f;
        public float fillSpeed = 5f;

        [Title("Автоматическое исчезновение")] public bool autoHideAMMO = true;
        ...
        private void Update()
        {
            ...
            if (!Mathf.Approximately(targetCurrentAmmo, lastCurrentAmmoValue) ||
                !Mathf.Approximately(targetTotalAmmo, lastAmmoValue))
            {
                ammoIdleTimer = 0f;
